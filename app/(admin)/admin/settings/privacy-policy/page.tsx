...
export default function AdminPrivacyPolicyPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await SettingService.update({ privacyPolicy: content });
      if (res.success) {
  ...
  return (
    <div className="space-y-8 max-w-5xl pb-16">
      <AdminPageHeader
        ...
        description="Edit the Privacy Policy displayed on the public website. Changes take effect immediately after saving."
      />
      ...
      {savedSuccess && (
        <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-900 text-xs font-semibold flex items-center gap-2.5 animate-in fade-in slide-in-from-top-2">
          ...
          <span>Privacy Policy saved successfully.</span>
        </div>
      )}
      ...
      <form onSubmit={handleSave} className="space-y-8">
        <Card className="p-6 md:p-8 bg-white border-slate-200 shadow-none space-y-4 rounded-xl">
          <div className="border-b border-slate-100 pb-4">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Write your full Privacy Policy below. Supports headings, lists, links, and tables.
            </p>
          </div>
        ...
        <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
          <p className="text-xs text-slate-600 font-medium">
            The saved content will appear on the public Privacy Policy page immediately.
          </p>
          <Button
            type="submit"
            disabled={saving}
