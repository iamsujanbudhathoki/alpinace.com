...
export function TrekkingCatalogClient({
  ...
  const isDefaultFilter =
    ...
    appliedMaxDuration >= (filterOptions?.maxDuration || 30) &&
    sortBy === "rating";
    ...
    async function loadTreks() {
      ...
      try {
        const raw = await TrekService.getPublicAll({
          ...
          maxDuration: appliedMaxDuration < (filterOptions?.maxDuration || 30) ? appliedMaxDuration : undefined,
          sortBy,
  ...
  }, [debouncedSearch, selectedCategory, selectedDifficulty, appliedMaxDuration, sortBy, filterOptions?.maxDuration, isDefaultFilter, initialTreks]);
  ...
  const resetFilters = () => {
    ...
    const defaultMax = filterOptions?.maxDuration || 30;
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (appliedMaxDuration < (filterOptions?.maxDuration || 30)) count++;
    if (searchQuery.trim() !== "") count++;
    return count;
  }, [selectedCategory, selectedDifficulty, appliedMaxDuration, searchQuery, filterOptions?.maxDuration]);
  ...
  const filteredTreks = useMemo(() => {
    ...
    if (appliedMaxDuration < (filterOptions?.maxDuration || 30)) {
    ...
    return list;
  }, [treks, debouncedSearch, selectedCategory, selectedDifficulty, appliedMaxDuration, sortBy, filterOptions?.maxDuration, isDefaultFilter]);
  ...
  const filterControls = (
    <div className="space-y-4">
      ...
      {(() => {
        const trekMinDuration = 3;
        const trekMaxDuration = filterOptions?.maxDuration || 30;
        const trekFillPct = Math.min(
          100,
          Math.max(0, ((localMaxDuration - trekMinDuration) / (trekMaxDuration - trekMinDuration)) * 100)
        );
        return (
          <div className="space-y-1.5">
            ...
            <div className="py-1">
              <input
                ...
                style={{
                  background: `linear-gradient(to right, #92400e 0%, #92400e ${trekFillPct}%, #e7e5e4 ${trekFillPct}%, #e7e5e4 100%)`,
                }}
            ...
            <div className="flex justify-between text-[10px] text-stone-500 font-medium">
              <span>{trekMinDuration} Days</span>
              <span>{trekMaxDuration} Days</span>
            </div>
          </div>
        );
      })()}
      ...
      <section className="bg-amber-50/70 border-b border-stone-200 pt-24 pb-10 sm:pt-28 sm:pb-12 relative overflow-hidden">
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-2">
          ...
          <p className="text-stone-600 text-sm font-normal leading-relaxed max-w-2xl">
            Explore guided trekking packages across Everest, Annapurna, Manaslu, and Langtang led by local mountain guides.
          </p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <main className="lg:col-span-8 space-y-6">
            <div className="flex items-center justify-between pb-3 border-b border-stone-200 text-xs sm:text-sm text-stone-600 font-medium">
              <span>Showing <strong className="text-stone-900 font-semibold">{loading ? "..." : filteredTreks.length}</strong> trekking routes</span>
              {activeFilterCount > 0 && (
                <button
                  onClick={resetFilters}
              ...
              <div className="bg-white border border-stone-200 rounded-md p-10 text-center space-y-3 max-w-md mx-auto my-4">
                ...
                <div className="space-y-1">
                  <h3 className="font-heading text-sm font-semibold text-stone-900">
                    {activeFilterCount > 0
                      ? "No treks match your selected filters."
                      : "No treks found"}
                  </h3>
                  <p className="text-stone-500 text-xs font-normal leading-relaxed">
                    {activeFilterCount > 0
                      ? "Try adjusting or clearing your filters to see more results."
                      : "There are currently no trekking packages available."}
                  </p>
                </div>
                {activeFilterCount > 0 && (
                  <button
                      ...
                      <div>
                        ...
                        <div className="p-4 space-y-2">
                          ...
                          <div className="pt-2 border-t border-stone-100 flex items-center justify-between text-xs text-stone-600">
                            <span>Max Altitude: <strong className="text-stone-900 font-semibold">{trk.maxAltitudeMeters ? `${trk.maxAltitudeMeters.toLocaleString()}m` : "High Altitude"}</strong></span>
                            <span>Grade: <strong className="text-stone-900 font-semibold">{trk.difficulty?.replace(" Trek", "") || "Moderate"}</strong></span>
                          </div>
                        </div>
                      </div>
