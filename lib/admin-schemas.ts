...
export const trekSchema = z.object({
  ...
  region: z.enum(["Everest", "Annapurna", "Langtang", "Manaslu"]),
  ...
  difficulty: z.enum(["Moderate Trek", "Challenging Trek", "Strenuous Trek"]),
  ...
  status: z.enum(["Active", "Featured", "Draft"]),
...
export const tourSchema = z.object({
  ...
  category: z.string().min(2, "Category is required"),
  region: z.enum(["Kathmandu & Pokhara", "Everest", "Annapurna"]),
  durationDays: z.preprocess((val) => Number(val), z.number().min(1, "Duration must be at least 1 day")),
...
export const expeditionSchema = z.object({
  ...
  category: z.string().min(2, "Category is required"),
  region: z.enum(["Everest", "Annapurna", "Manaslu"]),
...
export const bookingSchema = z.object({
  ...
  packageType: z.enum(["Trekking", "Expedition", "Tour"]),
  ...
  paymentStatus: z.enum(["Paid", "Deposit Paid", "Pending", "Refunded"]),
  bookingStatus: z.enum(["Confirmed", "In Review", "Active Trek", "Completed", "Cancelled"]),
  assignedGuide: z.string().optional(),
  permitStatus: z.enum(["Issued", "Processing", "Pending Document"]),
